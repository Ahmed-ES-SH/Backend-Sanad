...
export class ContactService {
  ...
  async remove(id: string): Promise<{ message: string }> {
    // Prevent deletion if total contact messages count is less than 5
    const totalCount = await this.contactMessageRepository.count();
    if (totalCount < 5) {
      throw new BadRequestException(
        'Cannot delete contact message when total count is less than 5',
      );
    }
  ...
  async getStats(): Promise<{
    ...
    const [total, read, unread] = await Promise.all([
      ...
      this.contactMessageRepository.count({ where: { isRead: true } }),
      this.contactMessageRepository.count({ where: { isRead: false } }),
    ]);
